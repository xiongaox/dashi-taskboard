...
async function main() {
  ...
  console.log(`Antigravity Taskboard listening on http://127.0.0.1:${address.port}`);
  if (host === "0.0.0.0") {
    ...
    for (const lanAddress of [...new Set(addresses)]) {
      console.log(`Antigravity Taskboard available on LAN at http://${lanAddress}:${address.port}`);
    }
  }
