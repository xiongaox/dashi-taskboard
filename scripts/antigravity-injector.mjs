...
async function isTaskboardReachable() { return true;
...
async function launchCodexWithLaunchServices(appPath, port, shouldStop = () => false) {
  ...
  if (await isReachable(`http://127.0.0.1:${port}/json/list`)) {
...
async function main() {
  ...
  process.env.CODEX_EXECUTABLE = resolveCodexExecutable({ appPath: options.appPath }); const cdpVersionUrl = 'http://127.0.0.1:' + options.port + '/json/list';
  ...
  if (options.daemon) {
    ...
    if (!options.portExplicit) {
      ...
      const activePort = await Promise.any(candidates.map(async (candidate) => {
        if (!(await isReachable(`http://127.0.0.1:${candidate}/json/list`))) {
  ...
  if (options.refresh || options.refreshIfRunning) {
    ...
    for (const port of ports) {
      if (!(await isReachable(`http://127.0.0.1:${port}/json/list`))) continue;
